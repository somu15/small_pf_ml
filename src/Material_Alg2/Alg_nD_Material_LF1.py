...
#     inpp = inp[None,:].reshape(1,Ndim)
#     y1[ii,0] = np.array(LS1.Material_HF1(inpp))
#     inp1[ii,:,0] = inp
#     print(ii)
...
for ii in np.arange(0,1):
    ...
    y1[indreq,ii] = ydiff[indreq,ii]+ylf[indreq,ii]
...
#     count_retrain = 0
#     count_retrain1 = 0
#     seeds_subs_info = subs_info[indices,kk-1]
#     while count_retrain < Ninit_GP:
#         ind_retrain = len(seeds_outs)-count_retrain1-1
#         inpp = seeds[ind_retrain,:].reshape(1,Ndim)
#         count_retrain1 = count_retrain1 + 1
#         if seeds_subs_info[ind_retrain] != 1:
...
#             GP_diff = InvNorm3(np.mean(np.array(samples1),axis=0),y_GPtrain)
#             y_nxt = np.array((LS1.Material_HF1(inpp))).reshape(1)
#             if y_nxt > 0.1 and int((y_nxt.reshape(1)-LF) in y_GPtrain)==0:
...
#                 amp_sto = np.concatenate((amp_sto, np.array(amp1).reshape(1)))
#                 len_sto = np.concatenate((len_sto, np.array(len1).reshape(1)))
#                 y1[indices[ind_retrain],kk-1] = y_nxt
#                 count_retrain = count_retrain + 1
#         if count_retrain1 >= int((Psub)*Nsub):
#             count_retrain = Ninit_GP
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        if u_check > u_lim: # and u_check1 >= u_lim and ii>500:
            y_nxt = LF + GP_diff
        else:
            y_nxt = np.array((LS1.Material_HF1(inpp))).reshape(1)
            if y_nxt > 0.1:
...
#     tmp1 = np.std(InvNorm3(np.array(samples1),y_GPtrain),axis=0)
#     y2_std[indreq,ii] = tmp1[indreq]

# y_tmp = np.zeros(Nsub)
# for ii in np.arange(2000,2500,1):
#     y_tmp[ii] = np.array(LS1.Material_HF(inp1[ii,:,2].reshape(1,Ndim))).reshape(1)
#     print(ii)
...
# plt.scatter(y_tmp[2000:2500],y1[2000:2500,2])
# plt.xlim([300,600])
# plt.ylim([300,600])
