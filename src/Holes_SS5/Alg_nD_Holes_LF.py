...
import pickle
os.chdir('/Users/dhulls/projects/Small Pf/Small_Pf_code/src/Holes_SS5')
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            ...
            # prop = np.exp(rv1.rvs())
            r = np.log(DR1.HolesPDF(rv_req=(prop), index=jj)) - np.log(DR1.HolesPDF(rv_req=(markov_seed[jj]),index=jj)) # rv.pdf((prop))/rv.pdf((inp1[ii-(int(Psub*Nsub)),jj,kk]))
...
# filename = 'SS_Run5.pickle'
# os.chdir('/home/dhullaks/projects/Small_Pf_code/src/Holes_SS5')
