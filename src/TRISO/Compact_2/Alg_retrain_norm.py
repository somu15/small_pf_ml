...
def Norm3(X1,X):
    return (X1-np.mean(X,axis=0))/(np.std(X,axis=0)) # (X1) # rv_norm.ppf(rv_out.cdf(X1)) #   /1.2e9

def InvNorm3(X1,X):
    return (X1*np.std(X,axis=0)+np.mean(X,axis=0)) # (X1) # rv_out.ppf(rv_norm.cdf(X1)) #  *1.2e9
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            ...
            # prop = (rv1.rvs())
            prop = markov_seed[jj] + (2*uni.rvs()-1)*std_prop[jj]
