...
ML0 = ML_TF(obs_ind = Norm1(inp_LFtrain,P,Ndim), obs = Norm3(y_HF_LFtrain,1))
...
ML = ML_TF(obs_ind = Norm1(inp_GPtrain,P,Ndim), obs = Norm3(y_GPtrain,1))
...
samples0 = ML0.GP_predict(amplitude_var = amp0, length_scale_var=len0, pred_ind = Norm1(inp1[ind_req,:,0].reshape(len(np.rot90(ind_req)),Ndim),P,Ndim), num_samples=500)
samples1 = ML.GP_predict(amplitude_var = amp1, length_scale_var=len1, pred_ind = Norm1(inp1[ind_req,:,0].reshape(len(np.rot90(ind_req)),Ndim),P,Ndim), num_samples=500)
y1[ind_req,0] = np.array(InvNorm3(np.mean(np.array(samples0),axis=0),1))+np.array(InvNorm3(np.mean(np.array(samples1),axis=0),1))
