...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  8 11:25:14 2021
...
inp_LFtrain = lhd
y_HF_LFtrain = Norm3(LS1.Triso_2d(inp_LFtrain),1)
...
y_HF_GP = Norm3(np.array((LS1.Triso_2d(inp_GPtrain))),1)
...
counter = 1
file1 = open('/home/dhullaks/projects/Small_Pf_code/src/Paper_T21_1_2dAlg/Results_retrain.csv','w')
...
for ii in np.arange(0,Nsub,1):
    ...
    if u_check > u_lim:
        ...
        y1[ii,0] = Norm3(np.array((LS1.Triso_2d(inpp))).reshape(1),1)
        ...
        subs_info[ii,0] = 1.0
    file1 = open('/home/dhullaks/projects/Small_Pf_code/src/Paper_T21_1_2dAlg/Results_retrain.csv','r')
...
for kk in np.arange(1,Nlim,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            rv1 = norm(loc=np.log(markov_seed[jj]),scale=prop_std[jj]) # prop_std[jj]
            prop = np.exp(rv1.rvs())
        ...
        samples0 = ML0.GP_predict(amplitude_var = amp0, length_scale_var=len0, pred_ind = Norm1(inpp,P,Ndim), num_samples=num_s)
        LF = np.array(InvNorm3(np.mean(np.array(samples0),axis=0),y_HF_LFtrain))
        samples1 = ML.GP_predict(amplitude_var = amp1, length_scale_var=len1, pred_ind = Norm1(inpp.reshape(1,Ndim),P,Ndim), num_samples=num_s)
        GP_diff = InvNorm3(np.mean(np.array(samples1),axis=0),y_GPtrain)
        ...
        if u_check > u_lim and u_check1 >= u_lim:
            ...
            y_nxt = Norm3(np.array((LS1.Triso_2d(inpp))).reshape(1),1)
...
filename = 'Alg_Run_retrain_24000.pickle'
os.chdir('/home/dhullaks/projects/Small_Pf_code/src/Paper_T21_1_2dAlg')
