...
Iters = 400
Ninit_GP = 5
lhd = DR1.FluidRandom(N=Ninit_GP)
...
for kk in np.arange(3,4,1):
    ...
    for ii in np.arange(0,(Nsub),1):
        ...
        for jj in np.arange(0,Ndim,1):
            ...
            prop = np.exp(rv1.rvs())
            r = np.log(DR1.FluidPDF(rv_req=prop, index=jj)) - np.log(DR1.FluidPDF(rv_req=(markov_seed[jj]),index=jj))
            if r>np.log(uni.rvs()):
                nxt[0,jj] = prop
            else:
        ...
        if kk<(Nlim-1):
            ...
            additive = value
        u_check = 3 # (np.abs(LF + GP_diff - additive))/np.std(InvNorm3(np.array(samples1),y_GPtrain),axis=0)
